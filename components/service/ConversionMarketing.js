	...
	return (
		<div className={`g-page_structure ${bodyColor}`}>
			<div className="m-auto pt-10 md:pt-20">
				<h1 className="text-[20px] md:text-4xl 2xl:text-[39px]  text-center mx-auto font-extrabold lg:w-[75%] xl:w-[70%] 2xl:w-[80%]">{title}</h1>
				<p className="md:text-lg xl:text-xl 2xl:text-2xl text-center md:px-48 m-auto py-4">{pera}</p>
			</div>
			<div className="md:mx-20 mt-10  flex flex-col gap-8">
				{paidAdvertising && (
					<Link href={"/"} className={`${cardColor} px-8 py-10 rounded-md shadow`}>
						<div className=" text-center ">
							<p className="font-extrabold text-xl md:text-2xl">Paid Advertising</p>
						...
						<Image src="/serveice/paid-advertising.svg" alt="" width={100} height={150} className="w-full h-[110px] md:h-[260px] mt-7" />
					</Link>
				)}
				{conversionRateOptimization && (
					<Link href={"/"} className={`${cardColor} px-8 py-10 rounded-md shadow`}>
