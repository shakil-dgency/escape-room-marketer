			...
			<div className={`work-together g-page_structure ${bodyColor}  relative`}>
				...
				<div className="q-card mx-0 md:mx-10 mb-8 mt-10 md:mt-20 grid gap-x-20 gap-y-12 md:grid-cols-1 relative">
					{keyServeiceData.card_content.first_card && (
						<div className={`px-10 py-10 md:col-span-2 rounded ${cardColor} shadow-md  grid md:grid-cols-2 items-center justify-items-center`}>
							<div className=" text-center ">
								<p className="font-circular font-bold text-xl">{keyServeiceData.card_content.first_card.heading}</p>
								<p className="font-graphik text-lg mt-2">{keyServeiceData.card_content.first_card.pera}</p>
							</div>
					...
					<Image
						...
						height={100}
						width={150}
						className="w-[100px] md:w-[150px] absolute -top-4 -left-6 md:-left-12 -rotate-45"
					/>
				</div>
					...
					{othersCard.map((data) => {
						return (
							<div className={`px-10 py-6  rounded ${cardColor} shadow-md`} key={data.heading}>
								<div className=" text-center ">
									<p className="font-circular font-bold text-xl">{data.heading}</p>
									<p className="font-graphik text-lg mt-2">{data.pera}</p>
								</div>
